# Lab19 - Monitoring Application Performance with Application Insights
...
On the Parts Unlimited web site, click the Brakes menu item.

In the URL textbox at the top of the browser window, append 1 to the end of the URL string and press Enter, effectively setting the CategoryId parameter to 11.
...
Return to the web browser tab displaying the Azure portal.

In the web browser tab displaying the Azure portal, on the App Service web app blade, in the vertical menu bar on the left, in the Settings section, click the Application Insights entry to display the Application Insights configuration blade.
...
On the Application Insights configuration blade, click the View Application Insights data link.

Review the resulting Application Insights blade displaying charts presenting different characteristics of the collected data, including the traffic you generated and failed requests you triggered earlier in this task.
...
On the Application Insights blade, in the vertical menu on the left side, in the Investigate section, click Smart Detection.
...
On the Application Insights blade, in the vertical menu on the left side, in the Investigate section, click Live Metrics.
...
Return to the web browser displaying the Parts Unlimited web site, navigate through its pages to generate some traffic, including a few server errors.

Return to the web browser displaying the Azure portal to watch the live traffic as it arrives.

On the Application Insights blade, in the vertical menu on the left side, in the Investigate section, click Transaction search.

> Note: Transaction search provides a flexible interface to locate the exact telemetry you need to answer questions.

On the Transaction search blade, click See all data in the last 24 hours.

> Note: The results include all telemetry data, which can be filtered down by multiple properties.

On the Transaction search blade, in the middle of the blade, right above the list of results, click Grouped results.

> Note: These results are grouped based common properties.

On the Transaction search blade, in the middle of the blade, right above the list of results, click Results to return to the original view, listing all results.

At the top of the Transaction search blade, click Event types = All selected, in the dropdown list, clear the Select all checkbox, and, in the list of event types, select the Exception checkbox.

> Note: There should be some exceptions representing the errors you generated earlier.

In the list of results, click one of them. This will display the End-to-end transaction details blade, providing a full timeline view of the exception within the context of its request.

At the bottom of the End-to-end transaction details blade, click View all telemetry.
...
Close the End-to-end transaction details blade and back on the Transaction search blade, in the vertical menu on the left side, in the Investigate section, click Availability.
...
On the Availability blade, in the toolbar, click + Add Classic test.

On the Create test blade, in the Test name textbox, type Home page, set the URL to the root of your App Service web app, and click Create.
...
On the Availability blade, in the vertical menu on the left side, in the Investigate section, click Failures.
...
On the Failures blade, in the upper right corner, in the Top 3 response codes list, click the link representing the number of 500 errors.
...
On the Failures blade, in the vertical menu on the left side, in the Investigate section, click Performance.
...
On the Performance blade, in the vertical menu on the left side, in the Monitoring section, click Metrics.
...
On the Metrics blade, in the filter section, click Select metric and, in the dropdown list, select Server requests.

> Note: You can also segment your data using splitting.

At the top of the newly displayed chart, click Apply splitting and, in the resulting filter, in the Select values dropdown list, select Operation name.
...
On the Users blade, below the main chart, click View More Insights. This will display additional data, extending the blade downwards.

Scroll down to review details about the geographies, operating systems, and browsers.

> Note: You can also drill into user-specific data to gain a better understanding of the user-specific usage patterns.

On the Users blade, in the vertical menu on the left side, in the Usage section, click Events.

On the Events blade, below the main chart, click View More Insights.
...
On the Events blade, in the vertical menu on the left side, in the Usage section, click Funnels.
...
On the Funnels blade, in the vertical menu on the left side, in the Usage section, click User Flows.
...
On the User Flows blade, click Select an event, on the Edit pane, in the Initial Event dropdown list, in the Page Views section, select Home Page - Parts Unlimited entry, and then click Create Graph.

On the User Flows blade, in the vertical menu on the left side, in the Usage section, click Retention.
...
On the Retention blade, click Retention Analysis Workbook, review the Overall Retention chart, and close the blade.

On the Retention blade, in the vertical menu on the left side, in the Usage section, click Impact.
...
On the Impact blade, click Impact Analysis Workbook, on the Impact blade, in the Selected event dropdown list, in the Page Views section, select Home Page - Parts Unlimited, in the Impacting event, select Browse Product - Parts Unlimited, review the results, and close the blade.

On the Impact blade, in the vertical menu on the left side, in the Usage section, click Cohorts.
...
On the Cohorts blade, in the vertical menu on the left side, in the Usage section, click More.

> Note: This blade includes a variety of reports and templates for review.

On the More | Gallery blade, in the Usage section, click Analysis of Page Views and review the content of the corresponding blade.
...
On the Alerts blade, in the toolbar, click + New alert rule.

On the Create alert rule blade, note that, in the Scope section, the current Application Insights resource will be selected by default.

On the Create alert rule blade, in the Condition section, click Add condition.

On the Configure signal logic blade, search for and select the Failed requests metric.

On the Configure signal logic blade, scroll down to the Alert logic section, ensure that Threshold is set to Static and set the Threshold value to 1.
...
On the Configure signal logic blade, click Done.

> Note: Now that the condition is created, we need to define an Action Group for it to execute.

Back on the Create alert rule blade, in the Action section, click Select action group and then, on the **Select an action group to attach to this alert rule, click + Create action group.
...
Back on the Notifications tab of the Create action group blade, in the Name textbox, type email and click Next: Actions >:

On the Actions tab of the Create action group blade, select the Action type dropdown list, review the available options without making any changes, and click Review + create.

On the Review + create tab of the Create action group blade, click Create

Back on the Create alert rule blade, in the Alert rule details section, in the Alert rule name textbox, type az400m17 lab alert rule, review the remaining alert rule settings without modifying them, and click Create alert rule.

Switch to the web browser window displaying the Parts Unlimited web site, on the Parts Unlimited web site, click the Brakes menu item.

In the URL textbox at the top of the browser window, append 1 to the end of the URL string and press Enter, effectively setting the CategoryId parameter to 11.

> Note: This will trigger a server error since that category does not exist. Refresh the page a few times to generate more errors.

After about five minutes, check your email account to verify that you have received an email indicating that the alert you defined was triggered.
